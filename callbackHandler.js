...
/**
 * Get user priority level for queue
 * @param {number} chatId - User Telegram ID
 * @returns {number} Priority level: 2=Owner/Admin, 1=Reseller, 0=Normal user
 */
function getUserPriorityLevel(chatId) {
    const adminIds = process.env.ADMIN_IDS?.split(',').map(id => id.trim()) || [];
    
    // Owner/Admin priority (level 2)
    if (adminIds.includes(String(chatId))) {
        return 2;
    }
    
    // Reseller priority (level 1)
...
/**
 * Get priority emoji based on user role
 * @param {number} chatId - User Telegram ID
...
/**
 * Send error log file from VPS logs directory
 * @param {TelegramBot} bot - Bot instance
 * @param {number} chatId - User chat ID
 * @param {string} errorContext - Context where error occurred (e.g., 'build', 'zip_build')
 * @returns {Promise<boolean>} - Returns true if log was sent, false if not found
 */
async function sendVpsErrorLog(bot, chatId, errorContext) {
    const logsBaseDir = '/home/webapp/logs/';
    ...
    try {
        ...
        // Filter file yang berakhiran .log.error
        ...
        // Cari file terbaru berdasarkan waktu modifikasi
...
/**
 * Clear/Clean old error log files (optional - untuk hapus file error lama)
 */
...
async function showQueueStatus(bot, chatId, messageId) {
    ...
    // Show active builds with user names and priority emoji
    if (activeBuilds.length > 0) {
        queueMessage += `\n<b>🔨 Sedang Berjalan:</b>\n`;
        for (const build of activeBuilds) {
            ...
            // Get priority emoji for active builds
            let activePriorityEmoji = '';
            if (build.priorityLevel === 2) activePriorityEmoji = '👑 ';
...
async function confirmBuild(bot, chatId, messageId) {
    ...
    const priorityEmoji = getPriorityEmoji(chatId);

    // Try to add to queue (will auto-queue if busy) with priority level
    const queueResult = buildQueue.addToQueue(chatId, buildData, 'url', userName, priorityLevel);
    ...
    if (!queueResult.immediate) {
        ...
        // Show priority badge based on role
        if (queueResult.isPriority) {
            if (priorityLevel === 2) {
                queueMessage += `👑 <b>PRIORITAS OWNER!</b> (Antrian prioritas tertinggi)\n\n`;
        ...
        if (activeBuilds.length > 0) {
            ...
            for (const build of activeBuilds) {
                const mins = Math.floor(build.duration / 60);
                let buildEmoji = '';
                if (build.priorityLevel === 2) buildEmoji = '👑 ';
                else if (build.priorityLevel === 1) buildEmoji = '⭐ ';
                queueMessage += `• ${buildEmoji}${escapeHtml(build.userName)} (${mins}m)\n`;
            }
            queueMessage += `\n`;
        }
        ...
        if (aheadOfMe.length > 0) {
            ...
            for (const item of aheadOfMe.slice(0, 5)) {
                let itemEmoji = '';
                if (item.priorityLevel === 2) itemEmoji = '👑 ';
                else if (item.priorityLevel === 1) itemEmoji = '⭐ ';
                const priority = item.priority ? '⚡ ' : '';
                queueMessage += `${item.position}. ${itemEmoji}${priority}${escapeHtml(item.userName)}\n`;
            }
...
5K 7H
10K 12H
Reseller 15K Permanen

Silahkan Scan Qr Di Profil Bot Kirimkan Ke Admin
💡 Hubungi @AanzCuyxzzz untuk mendapatkan akses.
        `.trim(), {
            parse_mode: 'HTML',
            reply_markup: getMainKeyboard()
        });
...
async function handleZipUpload(bot, chatId, filePath) {
    ...
    // Try to add to queue (will auto-queue if busy) with priority level
    const queueResult = buildQueue.addToQueue(chatId, buildData, 'zip', userName, priorityLevel);
